...
function MyTable({ search, data, getAllUsers}: any) {
              ...
              {page.map((row: any) => {
                ...
                return (
                  ...
                  >
                    ...
                    (
                      !(cell.column.id == "is_active") ? <td key={cell.column.id} className='h-10 pl-4 py-4 text-left truncate'> {/* Add truncate class */}
                        {cell.render('Cell')}
                      </td> :
                        <td key={cell.column.id} className=" box-border relative h-full w-full  items-center   py-4 sm:py-2 text-center flex gap-3 sm:gap-1 justify-between sm:flex-col">
                          <button className='rounded-[4px]   border-[1px] text-xs w-1/2 border-textY bg-bgY text-textY font-md px-2 py-1 sm:py-1 sm:px-1 hover:bg-opacity-20 sm:items-center sm:justify-center'
                                onClick={() => {
                                  localStorage.setItem("selectedUser",JSON.stringify(row.original))
                                  navigate('/dts/admin/user/update/')
                                }}>
                              ...
                              Swal.fire({
                                ...
                                if(clicked.isConfirmed){
                                  ...
                                  axios.get('users/user/' + userId + '/', {
                                      ...
                                      // Deactivate the user by changing the is_active to false
                                      ...
                                      axios.put('users/update/' + userId + '/', userData, {
                                        ...
                                        Swal.fire({
                                          icon: "success",
                                          title: "You have successfully deactivated this user!",
