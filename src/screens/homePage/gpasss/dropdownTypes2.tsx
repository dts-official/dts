...
const DropdownTypes = ({label,data,disabled,setData,onSubmit}:any) => {
  ...
  const [_otherOption, setOtherOption] = useState('');

  useEffect(()=>{
    setOtherOption(data.specific)
  ...
  return (
    <>
    <div className={" cursor-pointer col-span-2 sm:col-span-1 w-full flex flex-col gap-1" }>
        <p className=" text-xs">{label}</p>

        <Select disabled={disabled} required defaultValue={data.reason?data.reason:""}  onValueChange={handleChange} >
    ...
    {true  && (
        <div className=" col-span-2">
          <InputTemp type="text" label="Please specify" required placeholder="" value={data.specific} onChange={handleOtherChange} className="w-full p-2 border rounded" />
        </div>
      )}
    </>
  );
